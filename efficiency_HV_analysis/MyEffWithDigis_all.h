...
// Thu Mar 16 11:39:18 2017 by ROOT version 6.08/04
// from TTree DTTree/CMSSW DT tree
// found on file: /eos/cms/store/group/dpg_dt/comm_dt/dtRootple2017/Cosmics/DTTree_Run288825.root
/////////////////////////////////////////////////////////

#ifndef MyEffWithDigis_all_h
#define MyEffWithDigis_all_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   //TH1F *hPhiHitsSeg[5][4][14];
   TH1F *hAll[5][4][14];
   TH1F *hFound[5][4][14];
   TH1F *hFoundDigi[5][4][14];

   Float_t totalnumber[5][4][14];
   Float_t foundnumber[5][4][14];
   Float_t founddiginumber[5][4][14];
   ...
   //MyEffWithDigis_all(TTree *tree=0);
   MyEffWithDigis_all(TTree * /*tree*/ =0) : fChain(0) { }
   virtual ~MyEffWithDigis_all();
...
#ifdef MyEffWithDigis_all_cxx
/*
MyEffWithDigis_all::MyEffWithDigis_all(TChain *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/eos/cms/store/group/dpg_dt/comm_dt/dtRootple2017/Cosmics/DTTree_Run288825.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("/eos/cms/store/group/dpg_dt/comm_dt/dtRootple2017/Cosmics/DTTree_Run288825.root");
      }
...
MyEffWithDigis_all::~MyEffWithDigis_all()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t MyEffWithDigis_all::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t MyEffWithDigis_all::LoadTree(Long64_t entry)
{
...
void MyEffWithDigis_all::Init(TTree *tree)
{
   ...
   for(int wheel=0;wheel<5;wheel++){
	 ...
	 //hPhiHitsSeg[wheel][station][sector] = new TH1F(Form("hPhiHitsSeg%s",hName.Data()),hName,10,-0.5,9.5);
	 hAll[wheel][station][sector]        = new TH1F(Form("hAll%s",hName.Data()),hName,800,-4,4);
	 hFound[wheel][station][sector]      = new TH1F(Form("hFound%s",hName.Data()),hName,800,-4,4);
	 hFoundDigi[wheel][station][sector]  = new TH1F(Form("hFoundDigi%s",hName.Data()),hName,800,-4,4);
...
void MyEffWithDigis_all::Show(Long64_t entry)
{
...
Int_t MyEffWithDigis_all::Cut(Long64_t entry)
{
...
#endif // #ifdef MyEffWithDigis_all_cxx
